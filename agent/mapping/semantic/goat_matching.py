...
class GoatMatching(Matching):
    ...
    def get_matches_against_current_frame(
        ...
        **kwargs,
        ...
        # print("unprocessed view:", instance_memory.unprocessed_views[0])
        for local_instance_id, inst_view in instance_memory.unprocessed_views[
                0].items():
            if categories is not None and inst_view.category_id not in categories: # We do not consider the view whose category is different from our target
                continue
            if (inst_view.cropped_image.shape[0] *
                    inst_view.cropped_image.shape[1] < MIN_PIXELS or (np.array(
                        inst_view.cropped_image.shape[0:2]) < MIN_EDGE).any()):
                continue
            if use_full_image:
        ...
        try:
            return np.array([keypoints]), np.array([matches]), np.array(
                [confidences]), np.array([instance_ids])
        ...
        **kwargs,
        ...
        for (inst_key, inst) in instances.items():
            ...
            for view_idx, inst_view in enumerate(inst_views):
                if (inst_view.cropped_image.shape[0] *
                        inst_view.cropped_image.shape[1] < MIN_PIXELS
                        or (np.array(inst_view.cropped_image.shape[0:2])
                            < MIN_EDGE).any()):
                    continue
                if use_full_image:
                    img = instance_memory.images[0][
                        inst_view.timestep].cpu().numpy()
        ...
        if len(all_views) > 0:
            ...
            all_matches = np.split(all_matches,
                                   np.cumsum(instance_view_counts)[:-1])
            all_confidences = np.split(all_confidences,
                                       np.cumsum(instance_view_counts)[:-1])
        ...
        for i in tqdm(range(len(rgb_image_batched))):
            ...
            if isinstance(rgb_image_batched[i], np.ndarray):
                if rgb_image_batched[i].shape[0] == 3:
                    rgb_image_batched[i] = rgb_image_batched[i].transpose(
                        1, 2, 0)
    ...
    def match_image_batch_to_image(
        ...
        if use_full_image is not True:
            ...
            padding_bg = np.zeros(
                (max_detection_w, max_detection_h, 3), dtype=np.uint8) * 255
        ...
        return goal_keypoints, rgb_keypoints, matches.tolist(
        ), confidence.tolist()
    ...
    def match_language_to_image(self, views_orig, language_goal, **kwargs):
        ...
        if views[0].shape[0] == 3:
            views = torch.stack(
                [
                    self.clip_preprocess(ToPILImage()(v.transpose(
                        2, 1, 0).astype(np.uint8))) for v in views
                ],
                dim=0,
            )
        else:
            views = torch.stack(
                [
                    self.clip_preprocess(ToPILImage()(v.astype(np.uint8)))
                    for v in views
                ],
                dim=0,
            )
        ...
        view_embeddings = view_embeddings / view_embeddings.norm(dim=-1,
                                                                 keepdim=True)
        language_goal = language_goal / language_goal.norm(dim=-1,
                                                           keepdim=True)
        # compute cosines similarity
        similarity = (language_goal @ view_embeddings.T).squeeze(0)
        return [[[1]]] * similarity.shape[0], similarity.detach().cpu().numpy(
        ).reshape(-1, 1, 1)
    ...
    def get_best_match(self, scores, instance_ids, instance_map, score_thresh):
        ...
        idx = 0
        while (idx < len(sorted_inst_ids)
               and scores[sorted_inst_ids[idx]] > score_thresh):
            ...
            if instance_ids is None:
                print("=== Warning: no global instance ID!!! ===")
        ...
        if len(matches) > 0:
            for inst_idx, match_inst in enumerate(matches):
                ...
                for view_idx, match_view in enumerate(match_inst):
                    view_score = confidences[inst_idx][view_idx][match_view !=
                                                                 -1].sum()
    ...
    def get_goal_map_from_goal_instance(self, instance_map, goal_map,
                                        goal_inst, instance_goal_found,
                                        found_goal):
        lmb = [0, 0]  # Temporarily stored
        goal_pose = None
        if goal_inst is not None and instance_goal_found is True:
            ...
            if self.goto_past_pose:
                ...
                instance_views = instance_memory.instance_views[0][
                    goal_inst].instance_views
                # pick a view with maximum object coverage
                best_view = np.argmax(
                    [view.object_coverage for view in instance_views])
                ...
                inst_map_idx = torch.argmax(
                    torch.sum(inst_map_idx, axis=(1, 2)))
                goal_map = (instance_map[inst_map_idx] == goal_inst).to(
                    torch.float)
    ...
    def select_and_localize_instance(
        ...
        goal_pose = None
        instance_map = local_map[0].clone()

        if goal_inst is not None and instance_goal_found is True:
            goal_map, found_goal, goal_pose = self.get_goal_map_from_goal_instance(
                instance_map, goal_map, goal_inst, instance_goal_found,
                found_goal)
        ...
        if goal_inst is None and matches is not None:
            # print("=== goal is none and matched is not none ===")
            for e in range(confidence.shape[0]):
                scores = confidence[e]
                # print("[goat matching] scores length:", len(scores))
                if len(scores) > 0:
                    global_instance_ids = [
                        local_id_to_global_id_map[e].get(i, -1)
                        for i in local_instance_ids[e]
                    ]
                    # print("local instance id [from matching in obs_pre]:", local_instance_ids)
                    # print("global instance id:", global_instance_ids)
                    agg_scores = self.aggregate_scores_per_instance(
                        matches[e], confidence[e], agg_fn)
                    instance_goal_found, goal_inst = self.get_best_match(
                        agg_scores, global_instance_ids, instance_map,
                        score_thresh)
