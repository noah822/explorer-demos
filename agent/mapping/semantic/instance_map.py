...
from .instance_tracking_modules import InstanceMemory
...
class InstanceMapState:
    def __init__(
        ...
        # print("[instance_map] scene_size:",[self.scene_size_x, self.scene_size_y])
        ...
        self.obstacle_map_2d = torch.zeros(
            self.num_environments,
            self.scene_size_x,
            self.scene_size_y,
            device=self.device,
        ...
        self.topdown_category = torch.zeros(
            self.num_environments,
            self.scene_size_x,
            self.scene_size_y,
            device=self.device,
    ...
    def init_instance_map(self):
        '''Initialize instance and obstacle map'''
        for e in range(self.num_environments):
            self.instance_map_2d[e].fill_(0.0)
            self.obstacle_map_2d[e].fill_(0.0)
            self.topdown_category[e].fill_(0.0)
    ...
    def get_instance_map(self, e) -> np.ndarray:
        return np.copy(self.instance_map_2d[e].cpu().numpy())
    
    def get_obstacle_map(self, e):
        return np.copy(self.obstacle_map_2d[e].cpu().numpy())
    
    def get_topdown_map(self, e):
        return np.copy(self.topdown_category[e].cpu().numpy())


class InstanceMap(nn.Module):
    '''InstanceMap module for storing instance and obstacle map information'''
    def __init__(self,
                 scene_size: List[int],
                 num_sem_categories: int,
                 du_scale: int,
    ...
    def update_obstacle_map(
        ...
        old_obs_map: Tensor,
        new_obs_map: Tensor):
        '''Use logical or to update the obstacle map and return the result'''
        out = torch.logical_or(old_obs_map, new_obs_map)
        return out
    
    def update_topdown_cat(
        self,
        old_topdown: Tensor, # clone 
        new_topdown: Tensor): # clone
        '''
        Update the topdown map so that the old information is always kept.
        However, error could cascade.
        '''
        new_topdown[old_topdown != 0] = 0
        old_topdown += new_topdown
        return old_topdown
    ...
    def compute_coordinates(
        ...
        '''
        Compute the coordinate of a instance in the map. The final result is averaged across all points of the instance in the map.
        '''
        topdown4cat = topdown_map[env_id,category_id,:,:]
        ### TODO: locate the instance based on the instance id
        ### TODO: if we manage to locate the instance, return the center of all coordinates
        ### TODO: otherwise, we could not find the instance: an error has occured while projecting, and the instance is scraped off from memory
        coordinates_mask = topdown4cat == global_instance_id
        if coordinates_mask.any().item():
            coordinates = coordinates_mask.nonzero()
            results = [ 0 for _ in range(coordinates.shape[1])]
            for i in range(coordinates.shape[0]):
                for j in range(coordinates.shape[1]):
                    results[j] += coordinates[i,j]
            center = torch.floor( torch.tensor(results) / coordinates.shape[0] ).type(torch.int64)
            return center
        else:
    ...
    def forward(
        ...
        if num_instance_channels > 0:
            self.instance_memory.process_instances(
                ...
                pose.permute(1, 0),  # store the global pose
                image=obs[:, :3, :, :],
            )

        update_instance_map = self._aggregate_instance_map_channels_per_category(
            update_map, num_instance_channels)
        # print("aggregate_map,",update_instance_map.shape)

        updated_map = None
        for e in range(batch_size):
            # TODO if seq_update_global[e, t]:
            updated_map = self._update_global_map_for_env(e, update_instance_map,
                                            instance_map)
        assert torch.equal(updated_map, init_instance_map)
        return init_instance_map, seq_pose

    def _aggregate_instance_map_channels_per_category(self, curr_map,
                                                      num_instance_channels):
        ...
        if num_instance_channels > 0:
            ...
            for i in range(top_down_instance_one_hot.shape[0]):
                ...
                for instance_id, instance in unprocessed_instances.items():
                    category_id_to_instance_id_list[
                        instance.category_id].append(instance_id)
                ...
                for category_id in category_id_to_instance_id_list.keys():
                    ...
                    instance_one_hot = top_down_instance_one_hot[i,
                                                                 instance_ids]
                    ...
                    instance_id_map = torch.tensor(
                        instance_id,
                        device=instance_id_map.device)[instance_id_map]
                    # update the per category instance map
                    top_down_instances_per_category[
                        i, category_id] = instance_id_map
    ...
    def _update_global_map_instances(
        ...
        for i in range(self.num_sem_categories):
            if (torch.sum(updated_instance_map[e, i]) > 0):
                max_instance_id = (torch.max(
                    init_instance_map[e]).int().item())
                ...
                init_instance_map[e, i] = instances
    ...
    def _get_local_to_global_instance_mapping(
        ...
        for local_instance_id in local_instance_ids:
            ...
            overlapping_instances = global_instances_within_local[
                local_instance_pixels]
            ...
            if len(unique_overlapping_instances) >= 1:
                ...
                global_instance_id = int(
                    unique_overlapping_instances[0].item())
