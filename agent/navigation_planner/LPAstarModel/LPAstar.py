...
from LPAstarModel import env,plotting
#plotting


class LPAStar:
    def __init__(self, s_start, s_goal,cost_map:np.array, heuristic_type):
...
def load_traversible_map(path):
    with open(path, 'rb') as f:
        obstacle_map = np.load(f)
    return 1 - obstacle_map

def main():
    s_start = (55, 176)
    s_goal = (55, 218)
    traversible_map = load_traversible_map("/ludzy/example/dev/path_planning/traversible_map/obstacle1.npy")

    lpastar = LPAStar(s_start, s_goal,traversible_map, "Euclidean")
    print(lpastar.run())
    plt.show()
