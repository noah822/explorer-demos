...
from utils.geometry import xyt_global_to_base
...
class DiscretePlanner:
    ...
    def reset(self):
        ...
        self.col_width = 1
        self.last_pose = None
        # Need modification given different map
        # TODO
        self.curr_pose = [ 
            -9.6951,
            -1.1395,
    ...
    def plan(
        ...
        # ]
        start = np.array([sensor_pose[0], sensor_pose[1]])
        ...
        self.curr_pose = [sensor_pose[3], sensor_pose[4], sensor_pose[2]]
        self.visited_map[
        ...
        angle_agent = pu.normalize_angle(sensor_pose[2])
    ...
    def prepare_goal_inputs(
        ...
        self.last_pose = self.curr_pose
        obstacle_map = np.rint(obstacle_map)
        goal_map_tmp = np.copy(goal_map)

        start_pose = sensor_pose.copy()
        start = np.array([start_pose[0], start_pose[1]])
        start = pu.threshold_poses(start, obstacle_map.shape)
        start = np.array(start, dtype=int)
        ...
        if debug:
            ...
            print("Found goal:", found_goal)
            print("Goal points provided:", np.any(goal_map > 0))
        # print("[old_discrete_planner] start:", start)
        self.curr_pose = [start_pose[3], start_pose[4], start_pose[2]]
        self.visited_map[
            start[0] - 0 : start[0] + 1, start[1] - 0 : start[1] + 1
        ] = 1
        ...
        if self.last_action == "move_forward":
            print("[old_discrete_planner]: checking collision......")
            self._check_collision(*start)
        
        # Dilate obstacles & visited map
        # TODO: dilated map is not robust enough
        # dilated_obstacles = np.copy(obstacle_map)
        obstacle_map[self.collision_map == 1] = 1

        dilated_obstacles = cv2.dilate(obstacle_map, self.obs_dilation_selem, iterations=1)
        dilated_visited = cv2.dilate(self.visited_map, self.obs_dilation_selem, iterations=1)
        ...
        # traversible[self.visited_map == 1] = 1
        traversible[dilated_visited == 1] = 1

        plt.subplot(2,1,1)
        plt.imshow(dilated_visited, cmap='Greys', interpolation='nearest')
        plt.subplot(2,1,2)
        plt.imshow(traversible, cmap='Greys', interpolation='nearest')
        plt.savefig('datadump/images/visited_traversible.png')
        ...
        if not found_goal: # the goal map is just the frontier map, otherwise the obstacles can contain the goal
            goal_map_tmp[traversible == 0] = 0
        else: # if we found the goal, make the goal reachable
            goal_map_tmp = cv2.dilate(goal_map_tmp, self.obs_dilation_selem, iterations=1)
            traversible[goal_map_tmp == 1] == 1

        start_p = np.array([*start, start_pose[2]])
        
        if use_dilation_for_stg:
            dilated_goal_map = cv2.dilate(
                    goal_map_tmp, self.goal_dilation_selem, iterations=1
                )
            return traversible, dilated_goal_map, start_p, dilated_obstacles
        return traversible, goal_map_tmp, start_p, dilated_obstacles
    ...
    def get_action(
        ...
        if not (found_goal and stop):
            if self.discrete_actions:
                if relative_angle_to_stg > self.turn_angle:
                    action = "turn_right"
                elif relative_angle_to_stg < -self.turn_angle:
                    action = "turn_left"
                else:
                    action = "move_forward"
            else:
            ...
            if self.discrete_actions:
                if relative_angle_to_closest_goal > self.turn_angle:
                    action = "turn_right"
                elif relative_angle_to_closest_goal < -self.turn_angle:
                    action = "turn_left"
                else:
    ...
    def _get_short_term_goal(
        ...
        traversible[self.visited_map[x1:x2, y1:y2] == 1] = 1
        agent_rad = self.agent_cell_radius
        traversible[
        ...
        plt.savefig('datadump/images/debug/goal_map.png')
        planner = FMMPlanner(
            traversible,
    ...
    def _check_collision(self, start_x, start_y):
        ...
        if dist < self.collision_threshold: #0.2
            # We have a collision
            print("[old_discrete_planner] We have a collision!")
            self.collision_map[start_x:start_x+1, start_y:start_y+1] = 1
            ...
            #         )
            #         r, c = agent_pos_index = np.array([
            #             self._pos2index(axis, v) for v, axis in zip(
            #                 [wy, wx],
            #                 ['x', 'z'])
            #         ])
            #         # r, c = wy, wx
            #         print("[old_discrete_planner]",r,c)
    ...
    def _pos2index(self, coord: Union[str, List[str]],
        ...
        if type(value) == np.ndarray:
            ...
            return (100 * (value - coord_min) / self.map_resolution).astype(
                np.int32)
        else:
            return int(100 * (value - coord_str2min[coord]) / self.map_resolution)