...
class LanguageNavFrontierExplorationPolicy(nn.Module):
    ...
    def __init__(self, exploration_strategy: str, device_id = 0):
        ...
        self.dilate_explored_kernel = nn.Parameter(
            torch.from_numpy(skimage.morphology.disk(20))
            .unsqueeze(0)
            .unsqueeze(0)
    ...
    def reach_single_category(self, cat_map, exp_map, category, reject_visited_targets):
        # if the goal is found, reach it
        ### map_features -> category_map
        goal_map, found_goal = self.reach_goal_if_in_map(
            cat_map.unsqueeze(-1).permute(3,0,1,2), category, reject_visited_targets=reject_visited_targets
        )
        # otherwise, do frontier exploration
        ### map_features -> explored_map
        goal_map = self.explore_otherwise(exp_map, goal_map, found_goal)
    ...
    def forward(
        ...
        return self.reach_single_category(
            cat_map, exp_map, object_category, reject_visited_targets
        )
    ...
    def get_frontier_map_from_explored(self, explored_map):
        # Format the exolored map accordingly
        # TODO: need modification if batch size is above 1
        map_ = explored_map.unsqueeze(1).unsqueeze(1).permute(1,2,0,3)
        # Select unexplored area
        if self.exploration_strategy == "seen_frontier":
            frontier_map = (map_ == 0).float()
    ...
    def explore_otherwise(self, map_features, goal_map, found_goal):
        ...
        frontier_map = self.get_frontier_map_from_explored(map_features)
